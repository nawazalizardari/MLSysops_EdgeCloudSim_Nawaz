...
public class MobileHostEnergy extends MobileHost {
	...
	public MobileHostEnergy(int id, RamProvisioner ramProvisioner, BwProvisioner bwProvisioner, long storage,
			List<? extends Pe> peList, VmScheduler vmScheduler, DefaultEnergyComputingModel _energyModel,
			Double _batteryCapacity) {
		super(id, ramProvisioner, bwProvisioner, storage, peList, vmScheduler);
		energyModel = _energyModel;
//        batteryLevel = 500.0; // Math.round(Math.random() * 10000) / 100.0;
		batteryLevel = SimUtils.getRandomDoubleNumber(SimSettings.getInstance().getMIN_BATT_PERC(),
				SimSettings.getInstance().getMAX_BATT_PERC());
//        System.out.println("BATTERY LEVEL: "+batteryLevel);
	...
	/**
	 * this method is used to update the status of the device if the battery level
	 * is less than or equal to 0, the device is considered dead and the death time
	 * is set to the current time the static energy consumption is updated if the
	 * device is dead, it cannot do anything if the device is battery powered and
	 * the battery level is less than or equal to 0, the device is considered dead
	...
	/**
	 * this method is used to update the battery level of the device the battery
	 * level is updated by subtracting the energy consumed by the device if the
	 * battery level is less than the energy consumed by the device, the device is
	 * considered dead and the battery level is set to 0
	 */
...
//        return batteryLevel;
	 ...
	 *         <ul>
	 *         <li>MIPS (Million Instructions Per Second): Rappresenta la capacità
	 *         di elaborazione della VM, che influisce sul consumo energetico in
	 *         base alla quantità di lavoro svolto.</li>
	 *         <li>Numero di PEs (Processing Elements): Indica il numero di elementi
	 *         di elaborazione della VM, che può influenzare il consumo energetico
	 *         in base alla parallelizzazione delle operazioni.</li>
	 *         <li>RAM: La quantità di memoria RAM allocata per la VM, che influisce
	 *         sul consumo energetico in base alla gestione e all'accesso alla
	 *         memoria.</li>
	 *         <li>Bandwidth (Larghezza di banda): La larghezza di banda disponibile
	 *         per la VM, che influisce sul consumo energetico in base al
	 *         trasferimento di dati in rete o tra dispositivi.</li>
	 *         <li>Dimensione della VM: La dimensione della VM, che può influenzare
	 *         il consumo energetico in base alla gestione dello spazio su disco o
	 *         di altri dispositivi di memorizzazione.</li>
	 *         <li>Stato della VM: Indica se la VM è in fase di migrazione, che può
	 *         influenzare il consumo energetico in base alle operazioni necessarie
	 *         per la migrazione stessa.</li>
	 *         <li>Utilizzo totale della CPU: L'utilizzo totale della CPU da parte
	 *         della VM, che può influenzare il consumo energetico in base alla
	 *         quantità di lavoro eseguito.</li>
	 *         </ul>
	 *         </p>
	 */

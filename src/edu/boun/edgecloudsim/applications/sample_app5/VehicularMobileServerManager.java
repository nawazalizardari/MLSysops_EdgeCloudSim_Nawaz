...
public class VehicularMobileServerManager extends MobileServerManager {
	...
	public void createVmList(int brockerId) {
		...
		for (int i = 0; i < numOfMobileDevices; i++) {
			...
			// VM Parameters
			MobileVM vm = new MobileVM(vmCounter, brockerId, mips, numOfCores, ram, bandwidth, storage, vmm,
		...
		this.getDatacenter().getHostList().forEach(host -> {
			if (host instanceof MobileHostEnergy) {
				...
				if (!((MobileHostEnergy) host).isDead()) {
					...
					mapHostEnergyConsumed.put(host.getId(),
							"ENERGY CONSUMED HOST_ID[" + host.getId() + "] energy: " + ec);
				} else {

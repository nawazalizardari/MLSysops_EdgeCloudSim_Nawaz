...
public class VehicularEdgeServerManager extends EdgeServerManager {
		...
		for (int i = 0; i < datacenterList.getLength(); i++) {
			...
			for (int j = 0; j < hostNodeList.getLength(); j++) {
				...
				for (int k = 0; k < vmNodeList.getLength(); k++) {
					...
					// VM Parameters
					EdgeVM vm = new EdgeVM(vmCounter, brockerId, mips, numOfCores, ram, bandwidth, storage, vmm,
	...
	private List<EdgeHost> createHosts(Element datacenterElement) {
		...
		for (int j = 0; j < hostNodeList.getLength(); j++) {
			...
			// machines
			/*
			 * EdgeHost host = new EdgeHost(hostIdCounter, new RamProvisionerSimple(ram),
			...
			if (this.isEnergySimulation) {
				...
				DefaultEnergyComputingModel energyModel = new DefaultEnergyComputingModel(
						SimSettings.getInstance().getNumOfEdgeHosts(),
						SimSettings.getInstance().getEnergyConsumpitonMax_cloud(),
						SimSettings.getInstance().getEnergyConsumptionIdle_cloud());

				host = new EdgeHostEnergy(hostIdCounter, new RamProvisionerSimple(ram),
						new BwProvisionerSimple(bandwidth), // kbps
						storage, peList, new VmSchedulerSpaceShared(peList), energyModel, // todo inserire valori
						SimSettings.getInstance().getBATTERYCAPACITY());
			} else {
				host = new EdgeHost(hostIdCounter, new RamProvisionerSimple(ram), new BwProvisionerSimple(bandwidth), // kbps
				...
				if (host instanceof EdgeHostEnergy) {
					...
					if (!edgeHost.isDead()) {
						...
						mapHostEnergyConsumed.put(datacenter.getId(),
								"ENERGY CONSUMED HOST_ID EDGE[" + datacenter.getId() + "] " + "battery: "
										+ edgeHost.getBatteryLevel() + " energy: " + ec);
					} else {
